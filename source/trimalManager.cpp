...
void trimAlManager::parseArguments(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        // Trimming methods
        {
            ...
            checkArgument(complexPatternCleaning2)
        }
...
inline bool trimAlManager::patternCleaning(const int *argc, char *argv[], int *i) {

    if ((!strcmp(argv[*i], "-pattern")) && !patternTrim) {
        patternTrim = true;
        return true;
    }
    return false;
...
inline bool trimAlManager::complexPatternCleaning2(const int *argc, char *argv[], int *i) {

    if ((!strcmp(argv[*i], "-complexpattern2")) && !complexPatternTrim2) {
        complexPatternTrim2 = true;
        return true;
    }
    return false;
}
...
inline bool trimAlManager::check_thresholds_incompatibilities() {
        ...
        ) 
    {
        ...
        if (automatedMethodCount) {
            ...
            if (complexPatternTrim2)    autom = "-complexpattern2";
...
inline void trimAlManager::CleanResiduesAuto() {
    ...
    } else if (complexPatternTrim2) {
        tempAlig = singleAlig->Cleaning->cleanByComplexPattern2();
    }
